...
export const projectTranslations: Record<string, ProjectTranslation> = {
  ...
  lote: {
    title: 'Lote',
    blurb: '本地优先的 Markdown 笔记 + 无限画布桌面应用（Obsidian 风格）：笔记是存在你自己文件夹或 NAS 里的普通 .md / .canvas 文件，完全离线、无需账号、不锁定格式。',
    role: '独立开发',
    overviewGoal: '把写作、关联与可视化整理放进同一个桌面应用，同时让笔记保持为你完全自持、可随时迁移的纯文本文件。',
    overviewTeam: '独立产品',
    overviewTimeline: 'v3.0 · 2026',
    process: [
      { title: '问题', body: '云端笔记把数据锁进私有格式与服务器；本地工具又常常要装一堆插件才能凑齐功能。' },
      { title: '做法', body: '在你自选文件夹里的 .md / .canvas 之上，把双向链接、关系图谱、画布、看板与数据库视图开箱内置——本地优先、可离线。' },
      { title: '可用版本', body: '一个 Electron 桌面应用，打包为单文件便携 exe；直接读写本地或 NAS 文件夹，支持手动备份同步。' },
    ],
    technical: [
      {
        title: '纯文本·本地优先',
        description: '笔记是标准 Markdown 与 .canvas JSON，存在你自选的文件夹——本地磁盘或 NAS 共享——文件系统即数据库，不锁定云端。',
      },
      {
        title: '双向链接与图谱',
        description: '逐篇解析 [[链接]] 生成反向链接与 D3 力导向关系图；首行命名会自动改写所有指向该笔记的链接。',
      },
      {
        title: '数据库与看板视图',
        description: '同一批笔记可呈现为可排序的数据库表、看板、日历与大纲，读取标签与 frontmatter，全程保持纯文本。',
      },
      {
        title: '搜索·网页收藏·寿命',
        description: '全文、tag:、path:、正则与模糊搜索，外加网页收藏、回收站，以及把“临时想法”沉淀为“正式笔记”的笔记寿命。',
      },
    ],
    results: {
      summary: 'Lote 把写作、关联与可视化整理收进一个离线桌面应用，笔记始终是用户自持、可迁移的纯文本。',
      highlights: ['纯文本、本地优先', '双链/图谱/画布开箱内置', '单文件便携 Windows exe'],
    },
  },
  yinyang: {
    title: 'YinYang',
    blurb: '一个现代、可审计的八字数据仪表盘：精确历法、透明规则，每条结论都附置信度——按数据产品来做，而非算命软件。',
    role: '产品设计 / 前端开发',
    overviewGoal: '传统排盘网站界面陈旧、术语密集，还把不确定的推断当成定论。YinYang 保留东方气质，用现代数据产品的冷静重构八字。',
    overviewTeam: '独立产品',
    overviewTimeline: 'v1.0 · 2026',
    process: [
      { title: '问题', body: '传统八字工具界面杂乱、术语堆砌，结论黑箱，看不到推断如何得出。' },
      { title: '做法', body: '把命盘重构为结构化数据分析，界面克制现代——数据优先，计算与解读分离。' },
      { title: '信任', body: '每条结论都标注规则依据、古籍来源与置信度；整个引擎纯前端、可离线、无后端。' },
    ],
    technical: [
      {
        title: '精确排盘',
      ...
      {
        title: '五行结构',
        description: '天干与藏干加权后归一化到恰好 100，以进度条与雷达图呈现。',
      },
      {
        title: '可审计解读',
        description: '旺衰、用神、格局均为确定性计算，每层都带依据与置信度。',
      },
      {
        title: '带依据的推测',
        description: '事业、财富、感情、学业、健康、时机各自标注依据；健康注明非医疗。',
      },
      {
        title: '可互动·可嵌入',
        description: '今日运势、双人合盘、分享海报；数据存于 localStorage、无需账号，可通过 postMessage SDK 嵌入。',
      },
    ],
    results: {
      summary: '把密集命盘变成冷静可读的产品，同时不模糊计算与解读的边界——每条结论都能追溯到规则与来源。',
      highlights: ['天文级历法计算', '带依据与置信度的解读', '离线、可嵌入、无需账号'],
    },
  },
  shanhe: {
    title: '山河',
