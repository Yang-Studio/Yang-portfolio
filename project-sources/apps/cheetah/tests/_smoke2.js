...
global.React = require("react"); const RD=require("react-dom"); const RDC=require("react-dom/client");
global.ReactDOM = Object.assign({},RD,RDC);
const { act } = require("react");

// pre-seed a state with one plan due yesterday + a goal 1 step from done
const d = new Date(); d.setDate(d.getDate()-1);
const due = d.toISOString().slice(0,10);
const today = new Date().toISOString().slice(0,10);
store["leoledger_jsx_v3"] = JSON.stringify({
  welcomed:true,
  ...
  mascot:{xp:480,streakDays:12,lastEntry:today},
  pro:false
});
...
(async()=>{
  ...
  await click(planTab);
  const due0 = JSON.parse(localStorage.getItem("leoledger_jsx_v3")).planned[0].dueDate;
  const tx0 = JSON.parse(localStorage.getItem("leoledger_jsx_v3")).transactions.length;
  const dueBanner = root.textContent.includes("已到期");
  const chips=[...root.querySelectorAll(".pay-chip")];
  console.log("due banner shown:", dueBanner, "| pay-chips:", chips.length);
  await click(chips[chips.length-1]); // a row pay
  const st = JSON.parse(localStorage.getItem("leoledger_jsx_v3"));
  console.log("tx added:", st.transactions.length===tx0+1, `(${tx0}->${st.transactions.length})`);
  console.log("dueDate advanced:", st.planned[0].dueDate!==due0, `(${due0}->${st.planned[0].dueDate})`);
  console.log("balance changed:", st.accounts[0].balance);
  // goal completion -> celebration: open goals sheet via More tab
  const realErr = errors.filter(e=>!/not wrapped in act|ReactDOMTestUtils|deprecated/.test(e));
  console.log("real errors:", realErr.length); realErr.slice(0,4).forEach(e=>console.log("  ⚠",e.slice(0,140)));
