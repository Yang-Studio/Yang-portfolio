...
(function () {
  ...
  var NAV = [
    { grp: '主功能', items: [
      ...
      { id: 'oracle', ic: '卜', t: '天机问答', en: 'Rule Oracle' },
  ...
  function renderShell() {
    ...
    main.appendChild(el('div', { class: 'topbar' }, [
      el('div', {}, [ el('h1', { class: 'page-title' }, [t[0]]), el('div', { class: 'page-sub' }, [t[1]]) ]),
      el('div', { class: 'topbar-right' }, [
        accentSeg(),
        el('div', { class: 'date-pill' }, [dateText]),
        el('button', { class: 'icon-btn', title: '通知', onclick: function () { toast('暂无新通知 · 今日' + (TITLES[state.route] ? '' : '') + '气运已更新'); } }, [ '鈴', el('span', { class: 'dot' }) ]),
  ...
  function pageDaily(c) {
    ...
    var lede = el('div', { class: 'daily-lede' }, [
      el('h2', {}, [vd === '大吉' || vd === '中吉' ? '紫气东来 · 诸事顺遂' : (vd === '凶' ? '宜守为安 · 静待时机' : '平和持中 · 稳步而行')]),
      el('p', {}, ['今日「' + dd.ganzhi + '」增强 ' + dd.ganElement + '、' + dd.zhiElement + '，相对本盘喜用「' + favEls.join('、') +
        '」判断为「' + (fav > 0 ? '有利' : fav < 0 ? '不利' : '平稳') + '」。' + (fav > 0 ? '宜主动把握，主动而为则事半功倍。' : fav < 0 ? '宜降低并行、先理边界与沟通。' : '按既定计划稳定推进即可。')]),
      el('div', { class: 'yi-ji' }, [
        el('div', { class: 'box yi' }, [ el('div', { class: 'lab' }, ['宜']), el('div', { class: 'val' }, [fav >= 0 ? '祈福 · 签约 · 出行' : '静养 · 学习 · 规划']) ]),
        el('div', { class: 'box ji' }, [ el('div', { class: 'lab' }, ['忌']), el('div', { class: 'val' }, [fav >= 0 ? '动土 · 争讼' : '冒进 · 重大决策']) ])
      ])
    ]);

    var ring = gaugeSvg(score);
    var scoreTrend = fav > 0 ? '顺势上行' : fav < 0 ? '收敛守成' : '平稳推进';
    var gaugeCard = el('div', { class: 'card daily-hero' }, [
      el('div', { class: 'daily-score-panel' }, [
        el('div', { class: 'kicker' }, ['DAILY INDEX']),
        el('div', { class: 'score-row' }, [
          el('div', { class: 'gauge small' }, [ ring, el('div', { class: 'g-center' }, [
            el('div', { class: 'g-score' }, [String(score)]),
            el('div', { class: 'g-verdict' }, [vd])
          ]) ]),
          el('div', { class: 'score-copy' }, [
            el('div', { class: 'score-title' }, ['综合运势']),
            el('div', { class: 'score-state ' + (fav > 0 ? 'joy' : fav < 0 ? 'warn' : '') }, [scoreTrend]),
            el('div', { class: 'score-meta' }, [
              el('span', {}, ['日课 ' + dd.ganzhi]),
              el('span', {}, ['喜用 ' + favEls.join('')]),
              el('span', {}, ['日主 ' + chart.dayMaster.label])
            ])
          ])
        ]),
        el('div', { class: 'score-strip' }, [
          miniMetric('吉时', jiShiZhi + '时'),
          miniMetric('财位', DIR[caiEl] || '中宫'),
          miniMetric('贵人', '属' + (ZODIAC[guiBranch] || '虎'))
        ])
      ]),
      lede
    ]);
    ...
    var infoCard = el('div', { class: 'card' }, [
      el('div', { class: 'info-3' }, [
        infoCell('吉 时', jiShiZhi + '时', (SHICHEN[jiShiZhi] || '').split(' ')[1] || ''),
        infoCell('财 位', DIR[caiEl] || '中宫', caiEl + '为财'),
        infoCell('贵 人', '属' + (ZODIAC[guiBranch] || '虎'), guiBranch + (GAN_WX[guiBranch] ? '' : ZHI_WX[guiBranch]) + '相助')
      ])
    ]);
    var luckCard = el('div', { class: 'card' }, [
      kvRow('幸运色', el('span', {}, [ el('i', { class: 'swatch', style: 'background:' + EL_COLOR[colorEl].hex }), ' ' + EL_COLOR[colorEl].name ])),
      kvRow('幸运数字', luckyNum || '8 · 3'),
      kvRow('本命卦象', gua.s + ' ' + gua.n)
    ]);

    // trend (month / quarter / year)
    ...
    var trendSub = state.trendRange === 'month' ? '综合气运指数 · 流日推演' : state.trendRange === 'quarter' ? '综合气运指数 · 逐周推演' : '综合气运指数 · 逐月推演';
    var trendCard = el('div', { class: 'card' }, [
      el('div', { class: 'card-h' }, [ el('div', {}, [ el('h3', {}, [trendTitle]), el('div', { class: 'sub' }, [trendSub]) ]),
        el('div', { class: 'seg' }, [ segBtn('月', 'month'), segBtn('季', 'quarter'), segBtn('年', 'year') ]) ]),
    ...
    var feCard = el('div', { class: 'card' }, [
      el('div', { class: 'card-h' }, [ el('div', {}, [ el('h3', {}, ['五行能量']), el('div', { class: 'sub' }, ['日主' + chart.dayMaster.label + ' · 喜用 ' + favEls.join('')]) ]) ])
    ]);
    feOrder.forEach(function (e) { feCard.appendChild(barRow(e + ' ' + e, feMap[e] || 0, 'f-' + EL_CLASS[e], 'c-' + EL_CLASS[e])); });

    // 运势维度
    var dims = [
      ['事业 · 功名', dimScore(chart, fav, ['正官','七杀','正印','偏印'])],
      ['财帛 · 进益', dimScore(chart, fav, ['正财','偏财','食神'])],
      ['姻缘 · 情感', dimScore(chart, fav, ['正财','偏财','正官','七杀'])],
      ['健康 · 体魄', dimScore(chart, fav, ['比肩','劫财','正印','偏印'])],
      ['人际 · 贵人', dimScore(chart, fav, ['比肩','劫财','食神','伤官'])]
    ];
    var dimCard = el('div', { class: 'card' }, [ el('div', { class: 'card-h' }, [el('h3', {}, ['运势维度'])]) ]);
    dims.forEach(function (x) { dimCard.appendChild(scoreBar(x[0], x[1])); });

    // 今日一卦
    var g = guaOfDay(chart, dd);
    var guaCard = el('div', { class: 'card' }, [
      el('div', { class: 'card-h' }, [el('h3', {}, ['今日一卦'])]),
      el('div', { style: 'display:flex;gap:20px;align-items:center;margin-bottom:14px' }, [
        guaGlyph(g.lines, g.moving),
        el('div', {}, [ el('div', { class: 'gua-name' }, [g.name]), el('div', { class: 'gua-sub', style: 'margin-top:6px' }, [g.tris + ' · ' + vd]) ])
      ]),
    ...
    // 开运锦囊
    var tipCard = el('div', { class: 'card' }, [
      el('div', { class: 'card-h' }, [el('h3', {}, ['开运锦囊'])]),
      tipRow(EL_COLOR[colorEl].name[0], '佩戴' + EL_COLOR[colorEl].name + ' · ' + (DIR[caiEl] || '正南') + '向', '助旺' + jiShiZhi + '时贵人运', EL_CLASS[colorEl]),
      tipRow((favEls[1] ? favEls[1] : favEls[0]), '多近' + (favEls[1] || favEls[0]) + ' · 调候命局', '以喜用调候润命局', EL_CLASS[favEls[1] || favEls[0]]),
      tipRow('福', jiShiZhi + '时祈福 · 行善', '积德以厚载福报', 'earth')
    ]);
    var posterCard = el('div', { class: 'card poster-card' }, [
      el('div', { class: 'card-h' }, [el('div', {}, [el('h3', {}, ['分享海报']), el('div', { class: 'sub' }, ['生成当前命主今日运势卡片'])])]),
      el('p', { class: 'prose' }, ['包含四柱、日主、今日评分、幸运色与五行分布，适合保存或分享。']),
      el('button', { class: 'btn-gold', onclick: function () { createTianjiPoster({ chart: chart, analysis: an, score: score, verdict: vd, daily: dd, luckyColor: EL_COLOR[colorEl], colorEl: colorEl, luckyDir: DIR[caiEl] || '中宫', luckyTime: jiShiZhi + '时' }); } }, ['生成分享海报'])
    ]);

    c.appendChild(el('div', { class: 'section-tabs' }, [ el('div', { class: 'tab on' }, ['今日 · 流年运势']) ]));
    c.appendChild(el('div', { class: 'grid g-12-5', style: 'margin-bottom:18px' }, [ gaugeCard, el('div', { class: 'grid', style: 'gap:18px' }, [infoCard, luckCard]) ]));
    c.appendChild(el('div', { class: 'grid g-2', style: 'margin-bottom:18px' }, [trendCard, feCard]));
    c.appendChild(el('div', { class: 'grid g-3' }, [dimCard, hourCard, tipCard]));
    c.appendChild(el('div', { style: 'margin-top:18px' }, [guaCard]));
    c.appendChild(el('div', { style: 'margin-top:18px' }, [posterCard]));
    animateBars(c);
  }
